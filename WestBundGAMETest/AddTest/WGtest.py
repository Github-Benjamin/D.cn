...
def AddItems(WGid,itemid,Num):
    url = "http://115.xxx.62.13:5400/?cmd=item&value=%s&account=%s&id=%s"%(Num,WGid,itemid)
    print url
...
def AddHero(WGid,item):
    url = "http://115.xxx.62.13:5400/?cmd=hero&value=1&account=%s&structid=%s"%(WGid,item)
    html = requests.get(url).text
    if "success=true" in html:
        pass
    else:
...
WGid = "Benjamin@qq.com"
# AddItems(WGid,10001,30)

# 添加英雄 ，格式：heroid
for i in itemnum:
    data = i.split()
    AddItems(WGid, data[0], 9)
    # AddHero(WGid, data[0])